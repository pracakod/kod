/* ==========================================================================
   Lista — receipts.js
   Odpowiada za: Paragony
   - Dodawanie zdjęć (kamera/galeria) i OCR (Tesseract.js — ładowany leniwie)
   - Ręczne dodawanie/edycję danych (sklep, data, kwota, tagi)
   - Wyszukiwanie (sklep, data, kwota)
   - Archiwizacja przeglądu (sortowanie wg daty, grupowo)
   - Eksport: CSV, PDF (wydruk do PDF), zdjęcia (pobranie)
   - Integracja: Storage (persistencja), Bus (statystyki)
   ========================================================================== */
...
/* OCR — Tesseract.js ładowany na żądanie */
...
/* Narzędzia: obraz -> dataURL (kompresja) */
...
/* Parsowanie tekstu z OCR do pól formularza (heurystyka) */
...
function renderList() {
  ...
  for (const r of filtered) {
    ...
    const ph1 = document.createElement("span"); // dla spójności siatki
...
/* Podgląd i edycja pojedynczego paragonu (w tym zdjęcia) */
...
/* Obsługa OCR w dialogu */
async function startOCR() {
  const fileInput = qs("#ocr-file");
  const ta = qs("#ocr-text");
  const res = qs("#scanner-result"); // nie używany tutaj, ale pozostawiamy stałość UI
  ...
  try {
    ...
    // Zapamiętaj tymczasowo skompresowany obraz przy formularzu (dataset)
...
/* Zapis paragonu z formularza OCR/ręcznego */
...
/* Prezentacja podglądu wybranego pliku w dialogu OCR */
...
export const Receipts = {
  ...
  async init(Bus) {
    ...
    // Nadpisanie akcji z app.js (capture), aby korzystać ze Storage zamiast fallbacku
    qs("#btn-receipt-add-photo")?.addEventListener("click", (e) => {
      e.preventDefault(); e.stopPropagation();
      const dlg = qs("#dialog-ocr");
      if (!dlg) return;
      if (typeof dlg.showModal === "function") dlg.showModal(); else dlg.setAttribute("open", "");
      // Otwórz selektor pliku
