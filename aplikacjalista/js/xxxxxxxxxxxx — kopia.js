/* ==========================================================================
   Lista — vacations.js
   Odpowiada za: System Wakacji (plany wyjazdów)
   - Tworzenie planu (nazwa, daty, cel)
   - Lista rzeczy (kategorie, ilości, statusy: potrzebne/spakowane/do kupienia)
   - Szablony: Góry, Morze, Miasto, Biznes
   - Dodawanie do innych sekcji:
       • Checklista: zadania przed wyjazdem
       • Zadania: harmonogram (plan dnia z godzinami)
       • Zakupy: rzeczy do kupienia z listy
   - Harmonogram podróży (data, godzina, aktywność)
   - Notatki
   - Budżet (plan i wykonanie)
   - Pogoda (prognoza – pobranie online + tryb ręczny; offline działa w pełni)
   - Mapy i lokalizacje (lista punktów, szybkie otwarcie w Mapach)
   Persistencja: Storage._db.vacations (offline-first) + outbox do Supabase (tabela „vacations”)
   ========================================================================== */
...
/* Lokalna baza i outbox dla wakacji */
...
/* Zapewnij kubełek w Storage dla wakacji */
...
/* Szablony list rzeczy (przykładowe zestawy startowe) */
...
/* Pogoda — pobieranie z open-meteo (opcjonalnie, brak klucza; offline działa bez) */
...
export const Vacations = {
  ...
  async init(Bus) {
    ...
    qs("#vacations-list")?.addEventListener("click", (e) => {
      ...
      // Kliknięcie karty (poza przyciskami) — edycja
      if (e.target.closest(".card-actions")) return;
      this.openEditor(id);
    });

    // Usuwanie przez dłuższe przytrzymanie
    this._bindLongPressDelete();

    // Odświeżanie po synchronizacji
  ...
  renderList() {
    ...
    for (const v of filtered) {
      ...
      // Dodatkowy status pakowania (delikatna informacja)
      ...
      timer = setTimeout(() => {
        ...
        if (confirm("Czy przenieść plan do kosza?")) {
          ...
          if (idx >= 0) {
            ...
            try { Storage._saveDB?.(); } catch {}
            // do kosza w Storage (z typem 'vacations')
  ...
  /* ---------- Synchronizacja (opcjonalna do tabeli „vacations”) ---------- */
  ...
  async processOutbox() {
    ...
    for (const rec of box) {
      ...
      try {
        ...
        // brak tabeli lub inny błąd — pozostaw w kolejce
