/* ==========================================================================
   Lista — archive.js
   Odpowiada za: widok Archiwum i „Ostatnio usunięte” (kosz)
   - Renderowanie list archiwalnych (wszystkie sekcje) z filtrowaniem
   - Przywracanie elementów z archiwum i kosza
   - Trwałe usuwanie z archiwum i kosza
   - Informacja o czasie do automatycznego usunięcia (7 dni) w koszu
   Integracja: storage.js (lokalne + kolejka synchronizacji), Bus (zdarzenia)
   ========================================================================== */
...
/* Konfiguracja TTL kosza (zgrywa się z storage.js) */
const TRASH_TTL_MS = 7 * 24 * 60 * 60 * 1000;

/* Formatery */
const fmtDate = new Intl.DateTimeFormat("pl-PL", { dateStyle: "medium" });

/* Mapowanie typów na etykiety czytelne dla użytkownika */
const TYPE_LABEL = {
...
export const Archive = {
  ...
  /* API ułatwiające archiwizację z innych modułów (opcjonalnie) */
  ...
  renderArchive() {
    ...
    // Filtr: "", "checklist", "tasks", "shopping"
    const typeAllow = (() => {
    ...
    for (const rec of arr) {
      ...
      // Placeholder dla układu checkboxa (zachowujemy spójność siatki)
      ...
      // Placeholder dla uchwytu (spójność siatki)
  ...
  /* Aktualizacja liczników „pozostało X dni” bez pełnego renderowania */
...
/* Inicjalizacja samoczynna (na wypadek niezależnego załadowania) */
