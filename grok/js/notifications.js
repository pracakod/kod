/* ==========================================================================
   Lista — notifications.js
   Odpowiada za: system powiadomień
   - Preferencje: globalne i per sekcja (Checklista, Zadania, Zakupy, Ważne daty, Współdzielone)
   - Ciche godziny (od/do, także zakres nocny przez północ)
   - Powiadomienia lokalne (Notification API / Service Worker / snackbar)
   - Integracja:
       • „Ważne daty”: przypomnienia (dzień wcześniej 09:00 i w dniu wydarzenia 09:00)
       • Zadania: terminowe alerty (gdy zbliża się termin)
       • Współdzielenie: informacja o zmianach w listach współdzielonych
   - Offline-first: harmonogram działa w przeglądarce, bez serwera.
   ========================================================================== */
...
const LS_SEEN  = "lista:notifs:seen";   // zapamiętane wysłane zdarzenia (anty-spam)
...
function markSeen(kind, id, stamp) {
  ...
  // sprzątanie co zapisu: usuń pozycje starsze niż 30 dni
...
async function showNative(title, body, tag) {
  // Preferuj Service Workera (jeśli zarejestrowany) — lepsza spójność w PWA
  try {
  ...
  // Fallback na Notification API w oknie
...
/* Integracja z danymi — awaryjny odczyt legacy (gdy Storage nieaktualny) */
...
export const Notifications = {
  ...
  async init(Bus) {
    ...
    // Zastosuj preferencje do UI (jeśli kontrolki istnieją)
    this._applyPrefsToUI();

    // Prośba o uprawnienie, jeśli włączone i brak decyzji
    ...
    // Uruchom harmonogram (sprawdzenie co minutę)
    this._startScheduler();

    // Reakcja na odzyskanie łączności (nie wymagana do lokalnych powiadomień, ale pozwala na odświeżenie danych)
  ...
  /* Wysyłka powiadomienia z uwzględnieniem preferencji i cichych godzin */
  ...
  _startScheduler(immediate = false) {
    ...
    if (immediate) run();
    // Wyrównanie do początku minuty dla spójności
  ...
  async _tick() {
    ...
    // Zadania — alarm jeśli termin wchodzi w najbliższą minutę
    if (this._prefs.channels.tasks && this._prefs.global) {
      this._checkTasksDue(now, in60s);
    }

    // Ważne daty — przypomnienia 09:00 (dzień wcześniej i w dniu wydarzenia)
  ...
  _flattenTasksFromSnapshot() {
    ...
    // uzupełnij danymi legacy, jeżeli brak
    const legacy = readLegacyTasks();
    // scal po id (LWW nieistotne dla przypomnień)
    const byId = new Map();
  ...
  _checkTasksDue(now, in60s) {
    ...
    for (const t of tasks) {
      ...
      // Jeżeli czas wchodzi w najbliższą minutę
      if (dueMs >= now && dueMs < in60s) {
        const stamp = Math.floor(dueMs / 60000); // „znacznik minuty”
  ...
  _checkImportantDates(now) {
    ...
    // Godzina przypomnienia 09:00 ± 1 minuta
  ...
  /* UI (sekcja Opcje) — odtworzenie stanu pól na bazie prefs */
  _applyPrefsToUI() {
    try {
      const p = this._prefs;
      const map = {
  ...
  /* Uprawnienia do Notification API */
