...
export const Statistics = {
  ...
  _render(r) {
    ...
    const totalsEl = qs("#stats-totals");
    if (totalsEl) {
      totalsEl.innerHTML = `
        <div><strong>Rachunki:</strong> ${PLN.format(r.totals.receipts)}</div>
        <div><strong>Planowane wydatki:</strong> ${PLN.format(r.totals.planned)}</div>
        <div><strong>Otwartych zadań zakupowych:</strong> ${PLN.format(r.totals.planned_open)}</div>
      `;
    }
    const compEl = qs("#stats-comparisons");
    if (compEl) {
      const pctSign = (p) => p > 0 ? `+${p}%` : `${p}%`;
      compEl.innerHTML = `
        <div><strong>Poprzedni okres (rachunki):</strong> ${PLN.format(r.comparisons.prevPeriod.receipts.prev)} → ${PLN.format(r.comparisons.prevPeriod.receipts.current)} ${pctSign(r.comparisons.prevPeriod.receipts.changePct)}</div>
        <div><strong>Produktywność:</strong> ${r.comparisons.prevPeriod.productivity.prev} → ${r.comparisons.prevPeriod.productivity.current} ${pctSign(r.comparisons.prevPeriod.productivity.changePct)}</div>
        <div><strong>Prognoza (rachunki):</strong> ${PLN.format(r.comparisons.forecastNext.receipts)}</div>
        <div><strong>Prognoza (produktywność):</strong> ${r.comparisons.forecastNext.productivity}</div>
      `;
    }
  }
};
