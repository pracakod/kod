/* ==========================================================================
   Lista — barcode-scanner.js
   Odpowiada za: skanowanie kodów kreskowych/QR aparatem (BarcodeDetector),
   rysowanie nakładki, obsługę trybu ręcznego, mapowanie kod→nazwa produktu
   oraz integrację z formularzami Zakupów i Kart lojalnościowych.
   Zależności: brak zewnętrznych bibliotek (pure JS), opcjonalnie Bus.
   ========================================================================== */

"use strict";

/* Klucz mapowania kod→nazwa produktu (lokalnie) */
...
export const Barcode = {
  ...
  _anim: null,
  _lastDetect: 0,
  _purpose: "shopping", // 'shopping' | 'loyalty'
  _map: {},
  ...
  async init(Bus) {
    ...
    // Obsługa zamknięcia dialogu „X” i ESC zapewnia natywny <dialog>
  },

  /* Otwarcie skanera z określonym celem */
  ...
  _resizeOverlay() {
    ...
    // Wymiary na podstawie boundingClientRect (CSS piksele)
    const rect = video.getBoundingClientRect();
    canvas.width = Math.max(320, Math.floor(rect.width));
    canvas.height = Math.max(200, Math.floor(rect.height));
  },

  /* BarcodeDetector lub tryb ręczny */
  async _initDetector() {
    try {
  ...
  _renderManualEntryUI() {
    const res = qs("#scanner-result");
    if (!res) return;
    // Usuń poprzednią zawartość pomocniczą (jeśli istnieje)
  ...
  /* Pętla klatek: rysowanie nakładki + detekcja */
  _loop() {
    const canvas = qs("#scanner-overlay");
  ...
  /* Obsługa wykrytego kodu: UI + Bus + integracja formularzy */
  _onDetected({ code, format }) {
    ...
    // Emit globalne zdarzenie (dla modułów, np. Zakupy/Karty)
    try { this._bus.emit("barcode:detected", { code, format, purpose: this._purpose }); } catch {}

    // Nie zamykamy natychmiast; użytkownik decyduje o akcji.
    toast("Kod rozpoznany.");
  },

  _applyCodeToContext(code) {
    if (this._purpose === "shopping") {
      // Wypełnij nazwę produktu, jeżeli pusta, używając mapowania
  ...
  _mapProductFlow(code) {
    ...
    saveMap(this._map);
    // Auto-uzupełnienie formularza Zakupów (jeśli otwarty)
    const nameInput = qs("#shop-name");
  ...
  _saveLoyaltyCardFlow(code) {
    ...
    // Emituj do przyszłego modułu kart
...
/* Inicjalizacja samoczynna (jeśli moduł załadowany niezależnie) */
