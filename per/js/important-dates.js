/* ==========================================================================
   Lista — important-dates.js
   Sekcja „Ważne daty”: wydarzenia, urodziny, rocznice
   - CRUD zdarzeń (nazwa, data, kategoria, notatki)
   - Przypomnienia opcjonalne (X dni wcześniej) + integracja z Zadaniami
   - Widok listy z filtrowaniem; integracja z kalendarzem
   - Sterowanie powiadomieniami sekcji (przełącznik „Przypomnienia”)
   - Offline-first (Storage), synchronizacja przez storage.js
   ========================================================================== */
...
export const Dates = {
  ...
  _bus: { on() {}, emit() {} },
  _editingId: null,
  _cursor: new Date(), // dla kalendarza
  ...
  async init(Bus) {
    ...
    // Kalendarz — przechwycenie nawigacji i klików na dni
    ...
    // Odświeżanie po synchronizacji/aktualizacji
  ...
  /* Filtrowanie listy wg kategorii */
  _getFiltered() {
  ...
  renderList() {
    ...
    for (const d of arr) {
      ...
      const ph = document.createElement("span"); // dla siatki
  ...
  /* Kalendarz miesięczny (wariant modułu; niezależny od CalendarBasic) */
  renderCalendar() {
    const grid = qs("#calendar"); if (!grid) return;

    // Ustal miesiąc z bieżącego „kursora” CalendarBasic, jeśli dostępny
    try {
  ...
  /* Przeniesienie do Zadań — wybór projektu i utworzenie zadania z terminem */
  _ensureToTaskDialog() {
  ...
  _confirmToTask() {
    ...
    // Termin: data wydarzenia lub X dni wcześniej
  ...
  /* Sterowanie powiadomieniami dla sekcji z poziomu widoku „Ważne daty” */
  _toggleSectionReminders(on) {
    // Wyrównaj stan z przełącznikiem w Opcjach, aby użyć logiki notifications.js
    ...
    if (master) {
      ...
      // Jeżeli panel Opcji nie został jeszcze wyrenderowany
      try {
