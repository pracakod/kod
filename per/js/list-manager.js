/* ==========================================================================
   Lista — list-manager.js
   Odpowiada za: zarządzanie listami/projektami i ich kolorami
   Sekcje: Checklista (listy), Zadania (projekty), Zakupy (listy)
   Funkcje:
   - dodawanie/zmiana nazwy/koloru,
   - usuwanie,
   - zmiana kolejności (góra/dół),
   - aktualizacja selektorów w UI,
   - synchronizacja ze storage.js (jeżeli dostępny),
   - wsparcie trybu gościa przez modyfikację localStorage (lista:fallback).
   ========================================================================== */
...
/* Klucz danych legacy (Fallback z app.js) */
...
/* Kolory dostępne dla list/projektów (zgodne z themes.css) */
const COLORS = ["red","orange","yellow","green","teal","blue","indigo","purple","pink","brown","gray"];

/* Mapowanie sekcji -> właściwości w danych legacy i w storage.js */
const MAP = {
  checklist: {
    label: "lista",
    legacyKey: "checklist",
    legacyLists: "lists",
    legacyItems: "items",     // mapa list_id -> pozycje
  ...
  tasks: {
    ...
    legacyItems: "items",     // mapa project_id -> zadania
  ...
  shopping: {
    ...
    legacyKey: "shopping",
    legacyLists: "lists",
    legacyItems: "items",     // mapa list_id -> produkty
...
/* Odczyt/zapis legacy fallback (zgodny z app.js) */
function loadLegacy() {
  try {
...
/* Aktualizacja selektorów list/projektów w UI zgodnie z danymi legacy */
function refreshSectionSelect(section) {
  ...
  // przywróć poprzedni wybór (jeżeli istnieje), w przeciwnym razie pierwszy
  if (lists.some(l => l.id === prev)) sel.value = prev;
  else if (lists[0]) sel.value = lists[0].id;
}

/* Emit globalnego sygnału do ponownego renderu list */
function notifyUpdated() {
  try { window.Bus?.emit?.("list:updated"); } catch {}
}

/* Dialog zarządzania — tworzenie jednokrotne */
...
/* Rysowanie wiersza listy/projektu */
...
export const ListManager = {
  ...
  async init(Bus) {
    ...
    // Podpięcie przycisków „Zarządzaj listą/projektem”
    ...
    // Re-render selektorów po aktualizacji danych
    this._bus.on?.("storage:synced", () => {
      ["checklist","tasks","shopping"].forEach(refreshSectionSelect);
    });
  },

  /* Otwórz dialog i wyrenderuj bieżącą sekcję */
  open(section) {
  ...
  add(section) {
    ...
    // Synchronizacja ze storage.js (opcjonalnie)
    try {
  ...
  remove(section, id) {
    ...
    // Usuń także elementy należące do tej listy/projektu w legacy
    const itemsMap = bucket[map.legacyItems] || {};
    if (itemsMap[id]) delete itemsMap[id];

    saveLegacy(legacy);

    // storage.js — przeniesienie do kosza (soft delete)
    try {
      // Usuń encję listy/projektu (soft przez Storage.remove)
...
/* Inicjalizacja samoczynna (w przypadku niezależnego wczytania) */
