/* ==========================================================================
   Lista — statistics.js
   Odpowiada za: statystyki i raporty (wydatki, nawyki zakupowe, produktywność),
   zakresy czasu (dzień/tydzień/miesiąc/kwartał/rok/custom), eksport raportu,
   proste prognozy oraz porównania okres-do-okresu. Render bez bibliotek (Canvas 2D).
   ========================================================================== */
...
/* Pobranie kolorów wykresów z CSS */
function chartPalette(n = 6) {
  ...
  // Generowanie dodatkowych barw (odcienie koła barw)
  const extra = [];
...
function parseISODate(s) {
  // Akceptuj "YYYY-MM-DD" lub pełny ISO
  if (!s) return null;
...
function rangeForPreset(preset) {
  ...
  switch (preset) {
    ...
    default: {
      // custom — zostanie nadpisany w _getSelectedRange()
...
/* Czy timestamp (ms) mieści się w przedziale zamkniętym [from, to] */
...
/* Prostolinijne narzędzia do renderu Canvas (bez bibliotek) */
const Chart = {
  setup(canvas) {
...
function computeStats(snapshot, from, to) {
  const data = {
    ...
    expensesByCategory: [], // [{label, value}]
    storesTotals: [],       // [{label, value}]
    timeOfDay: [],          // [{label, value}] (rano/popołudnie/wieczór/noc)
    productivityByDay: [],  // [{label(YYYY-MM-DD), value}]
    comparisons: {
      prevPeriod: {
  ...
  // Kategorie (z tagów: dzielimy kwotę po równo między tagi, w braku tagów → "inne")
  ...
  // Pory dnia (na podstawie created_at/updated_at)
  ...
  /* 2) Planowane wydatki (Zakupy — koszt szacunkowy) */
  ...
  /* 3) Produktywność — liczba ukończonych zadań i odhaczonych pozycji */
  const days = [];
  ...
  // Uporządkuj po dacie w zakresie
  ...
  // Prognoza: średnia dzienna × długość następnego okresu (taka sama długość)
...
function exportReportCSV(report) {
  // Minimalny CSV: kategorie, sklepy, produktywność dzienna
  const rows = [];
...
export const Statistics = {
  ...
  _render(r) {
    ...
    // 2) Sklepy i pory dnia (słupki)
    const c2 = qs("#chart-habits");
    if (c2) {
      // Połączymy w jeden wykres słupkowy: najpierw top sklepy (do 5), potem pory dnia
    ...
    // 3) Produktywność (słupki dzienne)
    const c3 = qs("#chart-productivity");
    if (c3) {
      const labels = r.productivityByDay.map(x => x.label.slice(5)); // MM-DD
    ...
    // Dodatkowe podsumowanie (tekst) — wstawiane pod wykresami (jeśli chcemy)
    // Nie dodajemy nowych elementów DOM — pozostajemy przy płótnach i eksporcie.
  }
};

/* Inicjalizacja samoczynna (gdy moduł został dołączony niezależnie) */
